// src/components/GroupPanel/GroupPanel1.tsx
import React, { useRef, useState, useEffect } from 'react';
import { ColDef } from 'ag-grid-community';
import { GroupPanelProps } from '../types';
import AvailablePanel from './components/AvailablePanel';
import SelectedFlatPanel from './components/SelectedFlatPanel';

const GroupPanel1: React.FC<GroupPanelProps> = ({
  groupsCols,
  selectedGroups,
  onGroupChanged
}) => {
  ...
  const handleDrop = (e: React.DragEvent<HTMLDivElement>, panel: string, groupPath?: string) => {
    ...
    } else if (dragData.type === 'group_path' && panel === 'selected') {
      // Move all groups in the path to selected
      const { groupPath: draggedPath } = dragData;
